...
mineos.mc = function (server_name, base_dir) {
  ...
  self.create_from_archive = function (owner, filepath, callback) {
    ...
    if (filepath.match(/\//))
      ...
      var dest_filepath = filepath; // if it doesn't treat it as being from /import/
  ...
  self.get_start_args = function (callback) {
      ...
      async.series(
        ...
        function (err, results) {
          if (err) {
            ...
            var args = ["-dmSL", "mc-{0}".format(self.server_name)];
      ...
      async.series(
        ...
        function (err, results) {
          if (err) {
            ...
            var args = ["-dmSL", "mc-{0}".format(self.server_name), results.binary, results.pharfile];
            inner_callback(null, args);
          }
        },
      );
    ...
    function type_cuberite(inner_callback) {
      var args = ["-dmSL", "mc-{0}".format(self.server_name), "./Cuberite"];
      inner_callback(null, args);
    }
